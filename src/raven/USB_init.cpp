...
//#include <ros/console.h> old ros library updated with rclcpp MSK
  ...
  for (uint i = 0; i < files.size(); i++) {
    ...
    if ((boardid == GREEN_ARM_SERIAL) || (boardid == GOLD_ARM_SERIAL) ||
      ...
      if (ioctl(tmp_fileHandle, BRL_RESET_BOARD) != 0) {
        RCLCPP_ERROR("ERROR: ioctl error opening board %s", boardStr.c_str()); //switched from ros to rclcpp library MSK
        errno = 0;
      }
      ...
      if (write_zeros_to_board(boardid) != 0) {
        RCLCPP_ERROR("Warning: failed initial board reset (set-to-zero)"); //switched from ros to rclcpp library MSK
      }
    }
  }

  if (okboards < 2) {
    RCLCPP_ERROR( //switched from ros to rclcpp library MSK
