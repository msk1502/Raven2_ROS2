...
void *console_process(void *) {
  rclpp::Time t1, t2; //changed from ros namespace to rclpp MSK
  rclpp::Duration d;
  t1 = t1.now();
  t2 = t2.now();
  ...
  while (rclpp::ok()) { //switched ros::ok to rclpp::ok MSK
    // Output UI hints
    if (print_msg) {
...
// Get user-input DAC value #
    ...
    while (msgqueue.size() > 0) {
      ...
      // std::cout << "buf\n" << buf << std::endl;
      RCLPP_INFO("%s", buf); //switched ROS_INFO to RCLPP_INFO MSK
      msgqueue.pop();
      free(buf);
    }
  }
