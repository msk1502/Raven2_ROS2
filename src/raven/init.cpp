...
//#include <ros/console.h> from ros noetic
 ...
 *      Make sure that numerical order of parameters matches the numerical order
 of the dof types
  ...
  \ingroup DataStructures
*/
int init_ravengains(rclcpp::Node n, device *device0) { //changed passing of nodehandle n to rclcpp::Node n MSK
  ...
  /// ain't found
  /// changed hasParam to has_parameter and getParam to get_parameter MSK 
  if (n.has_parameter("/gains_gold_kp") && n.has_parameter("/gains_gold_kd") &&
      n.has_parameter("/gains_gold_ki") && n.has_parameter()"/gains_green_kp") &&
      n.has_parameter("/gains_green_kd") && n.has_parameter("/gains_green_ki")) {
    res = n.get_parameter("/gains_green_kp", kp_green);
    res &= n.get_parameter("/gains_green_kd", kd_green);
    res &= n.get_parameter("/gains_green_ki", ki_green);
    res &= n.get_parameter("/gains_gold_kp", kp_gold);
    res &= n.get_parameter("/gains_gold_kd", kd_gold);
    res &= n.get_parameter("/gains_gold_ki", ki_gold);
  }
  ...
  if (!res || (kp_green.size() != MAX_DOF_PER_MECH) || (kd_green.size() != MAX_DOF_PER_MECH) ||
    ...
    RCLPP_ERROR("Gains parameters failed.  Setting zero gains"); //changed all ROS_ERROR to RCLCPP_ERROR MSK
  } else {
    ...
    for (int i = 0; i < NUM_MECH; i++) {
      for (int j = 0; j < MAX_DOF_PER_MECH; j++) {
        ...
        if (device0->mech[i].type == GOLD_ARM) {
          ...
          RCLPP_ERROR("What device is this?? %d\n", device0->mech[i].type);
        }
      }
    }
    if (!initgold) {
      RCLPP_ERROR("Failed to set gains for gold arm (ser:%d not %d).  Set to zero",
                device0->mech[0].type, GOLD_ARM);
    }
    if (!initgreen) {
      RCLPP_ERROR("Failed to set gains for green arm (ser:%d not %d).  Set to zero",
