...
#include "sensor_msgs/msg/JointState.hpp" //updates from ROS Noetic Jointstate to ROS2 MSK 

void publish_joints(robot_device *);
void autoincrCallback(raven_2::raven_automove);

using namespace raven_2; //need to edit namespace MSK
// Global publisher for raven data

//moved declaration into the init
 ...
 *initializer
 */
int init_ravenstate_publishing(rclcpp::Node &n) {
  //old ross code
  ...
  //                                                                 .reliable());
  auto pub_ravenstate = n->create_publisher<raven_state>("ravenstate, 1000")
  auto joint_publisher = n->create_publisher<sensor_msgs::JointState>("joint_states", 1)
  auto sub_automove = n->create_subscription<raven_automove>("raven_automove", 1, autoincrCallback)
...
void publish_ravenstate_ros(robot_device *dev, param_pass *currParams) {
  ...
  //update library to rclcpp MSK
  static rclcpp::Time t1;
  static rclcpp::Time t2;
  static rclcpp::Duration d;
  ...
  pub_ravenstate->publish(msg_ravenstate) //updated to ROS2 publish  MSK
}
...
void publish_joints(robot_device *device0) {
  ...
  joint_publisher->publish(joint_state) //updated to ros2 publish MSK
}
