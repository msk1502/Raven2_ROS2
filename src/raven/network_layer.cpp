...
void *network_process(void *param1) {
  ...
  if (logFile < 0) {
    RCLCPP_ERROR("ERROR: could not open log file. %d -- %s\n", logFile, std::strerror(errno)); //updated library to RCLCPP MSK
    exit(1);
  }
  gettimeofday(&tv, &tz);
  ...
  sock = initSock(port);
  if (sock <= 0) {
    RCLCPP_ERROR("socket: service failed to initialize socket. (%d)\n", logFile); //updated library to RCLCPP MSK
    exit(1);
  }
  ...
  while (ros::ok()) {
    ...
    {                            // to the fdset mask
      ...
      if (bytesread != uSize)
      {
        RCLCPP_ERROR("ERROR: Rec'd wrong ustruct size on socket!\n"); //updated library to RCLCPP MSK
      ...
      else if (u.sequence > seq + 1)  // Skipping sequence number (dropped)
      {
        ...
        sprintf(logbuffer, "%s Skipped (dropped?) packets %d - %d\n", ctime(&(tv.tv_sec)), seq + 1,
                u.sequence - 1);
        RCLCPP_ERROR("%s Skipped (dropped?) packets %d - %d\n", ctime(&(tv.tv_sec)), seq + 1, //updated library to RCLCPP MSK
                  u.sequence - 1);
        retval = write(logFile, logbuffer, strlen(logbuffer));
        seq = u.sequence;
      ...
      else if (u.sequence == seq)  // Repeated sequence number
      {
        ...
        RCLCPP_ERROR("%s Duplicated packet %d - %d\n", ctime(&(tv.tv_sec)), seq, u.sequence); //updated library to RCLCPP MSK
        retval = write(logFile, logbuffer, strlen(logbuffer));
      ...
      else 
      {
        ...
        RCLCPP_ERROR("%s Out of sequence packet %d\n", ctime(&(tv.tv_sec)), seq); //updated library to RCLCPP MSK
        retval = write(logFile, logbuffer, strlen(logbuffer));
      }
    }
