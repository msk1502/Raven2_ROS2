...
//#include <ros/ros.h>  old ros library updated to rclcpp MSK
...
static void *rt_process(void *) {
  ...
  while (rclcpp::ok() && !r2_kill) { //updated library to rclcpp MSK
    // Initiate USB Read
...
int init_ros(int argc, char **argv) {
  ...
  */
  ...
  auto node = rclcpp::Node::make_shared("r2_control")

  init_ravenstate_publishing(node);
  init_ravengains(node, &device0);
