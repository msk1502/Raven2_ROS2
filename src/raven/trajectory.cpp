...
//#include <ros/ros.h> old ros library updated with rclcpp
...
int update_linear_sinusoid_position_trajectory(DOF *_joint) {
  ...
  rclcpp::Duration t = rclcpp::Time::now() - traj->startTime; //switched from ros to rclcpp libraries MSK
...
int update_position_trajectory(DOF *_joint) {
  ...
  rclcpp::Duration t = rclcpp::Time::now() - traj->startTime; //switch from ross to rclcpp library MSK
